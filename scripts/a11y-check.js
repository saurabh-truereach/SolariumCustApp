...
function runESLintA11yChecks() {
  ...
  try {
    ...
    if (a11yIssues.length === 0) {
      ...
      console.log(`❌ Found ${a11yIssues.length} accessibility linting issues:\n`);
      
      a11yIssues.forEach(issue => {
        const severity = issue.severity === 'error' ? '❌' : '⚠️';
        console.log(`${severity} ${path.relative(process.cwd(), issue.file)}:${issue.line}:${issue.column}`);
...
function checkTouchTargets() {
  ...
  touchableComponents.forEach(component => {
    const tooSmall = component.width < minimumSize.width || component.height < minimumSize.height;
    ...
    if (tooSmall) {
      ...
      console.log(`❌ ${component.name}: ${component.width}x${component.height}px (minimum: ${minimumSize.width}x${minimumSize.height}px)`);
    } else {
      console.log(`✅ ${component.name}: ${component.width}x${component.height}px`);
    }
  });
...
function calculateOverallScore(results) {
  ...
  const contrastIssues = results.colorContrast?.filter(c => !c.passes).length || 0;
...
<body>
  ...
  <div class="section">
    <h2>Summary</h2>
    <table>
      <tr><td>Critical Issues</td><td class="${report.criticalIssues > 0 ? 'fail' : 'pass'}">${report.criticalIssues}</td></tr>
      <tr><td>Warnings</td><td class="${report.warnings > 0 ? 'warning' : 'pass'}">${report.warnings}</td></tr>
      <tr><td>Color Contrast Issues</td><td class="${report.contrastIssues > 0 ? 'fail' : 'pass'}">${report.contrastIssues}</td></tr>
      <tr><td>Touch Target Issues</td><td class="${report.touchTargetIssues > 0 ? 'warning' : 'pass'}">${report.touchTargetIssues}</td></tr>
      <tr><td>Tests Pass</td><td class="${report.testsPass ? 'pass' : 'fail'}">${report.testsPass ? 'Yes' : 'No'}</td></tr>
    </table>
  </div>
  
  <div class="section">
    <h2>Color Contrast Results</h2>
    ${report.details.colorContrast?.map(c => `
  ...
  <div class="section">
    ...
    <ul>
      ${report.criticalIssues > 0 ? '<li>Fix critical accessibility issues before deployment</li>' : ''}
      ${report.contrastIssues > 0 ? '<li>Improve color contrast ratios for better readability</li>' : ''}
      ${report.touchTargetIssues > 0 ? '<li>Increase touch target sizes to meet minimum requirements</li>' : ''}
      ${!report.testsPass ? '<li>Fix failing accessibility tests</li>' : ''}
      ${report.overallScore === 100 ? '<li>Excellent! All accessibility checks passed</li>' : ''}
    </ul>
  </div>
</body>
</html>
